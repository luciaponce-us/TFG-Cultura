...
class RolSagaControllerTest extends BaseControllerTest {
    ...
    void should_create_rol_saga() throws Exception {
        when(rolSagaService.create(any(RolSagaRequest.class), any()))
                .thenReturn(rolSagaResponse);
        ...
        mockMvc.perform(multipart(BASE_URL)
                .file(rolSagaPart)
                .file(imagePart).contentType(MediaType.MULTIPART_FORM_DATA))
    ...
    void should_create_rol_saga_without_image() throws Exception {
        when(rolSagaService.create(any(RolSagaRequest.class), isNull()))
                .thenReturn(rolSagaResponse);

        MockMultipartFile rolSagaPart = mockRolSagaPart(rolSagaRequest);

        mockMvc.perform(multipart(BASE_URL)
                .file(rolSagaPart).contentType(MediaType.MULTIPART_FORM_DATA))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id").value(rolSagaResponse.getId()))
                .andExpect(jsonPath("$.name").value(rolSagaResponse.getName()))
                .andExpect(jsonPath("$.description").value(rolSagaResponse.getDescription()))
    ...
    void should_return_rol_saga_by_id() throws Exception {
        when(rolSagaService.getById(anyString()))
                .thenReturn(rolSagaResponse);

        mockMvc.perform(get(BASE_URL + "/{id}", rolSagaResponse.getId()))
    ...
    void should_return_all_rol_sagas() throws Exception {
        ...
        Page<RolSagaResponse> page = new PageImpl<>(
                List.of(rolSagaResponse),
                PageRequest.of(0, 10),
                1);
    ...
    void should_update_rol_saga() throws Exception {
        ...
        mockMvc.perform(multipart(BASE_URL + "/{id}", rolSagaResponse.getId())
                .file(rolSagaPart)
                .file(imagePart)
                .with(request -> {
    ...
    void should_return_not_found_when_updating_non_existent_rol_saga() throws Exception {
        ...
        mockMvc.perform(multipart(BASE_URL + "/{id}", "non-existent-id")
                .file(rolSagaPart)
                .with(request -> {
                    request.setMethod("PUT");
                    return request;
    ...
    void should_delete_rol_saga() throws Exception {
        mockMvc.perform(delete(BASE_URL + "/{id}", rolSagaResponse.getId()))
