...
public abstract class BaseControllerTest {
	...
	protected MockMvc buildMockMvc(Object controller, Class<?>... extraAdvices) {
		...
		return MockMvcBuilders.standaloneSetup(controller).setValidator(validator)
				.setControllerAdvice(advices.toArray()).build();
	}

	private Object createAdvice(Class<?> adviceClass) {
		try {
			// Asume constructor con ApiErrorBuilder
			return adviceClass.getConstructor(ApiErrorBuilder.class).newInstance(apiErrorBuilder);
		} catch (Exception e) {
			throw new RuntimeException("No se pudo instanciar el advice: " + adviceClass.getSimpleName(), e);
		}
	}
}
