...
class ItemControllerTest extends BaseControllerTest {
	...
	void should_return_item_types() throws Exception {
		...
		mockMvc.perform(get(ITEM_TYPES_URL)).andExpect(status().isOk()).andExpect(jsonPath("$[0]").value("Book"))
				.andExpect(jsonPath("$[1]").value("Movie")).andExpect(jsonPath("$[2]").value("Music"));
	}
