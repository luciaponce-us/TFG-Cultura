...
class RolGameControllerTest extends BaseControllerTest {
	...
	void should_find_all_by_saga_id() throws Exception {
		when(rolGameService.findAllBySagaId("saga-id")).thenReturn(List.of(rolGameResponse));

		mockMvc.perform(get(ROL_GAMES_BY_SAGA_URL, "saga-id")).andExpect(status().isOk())
	...
	void should_return_empty_list_when_no_rol_games_found_for_saga() throws Exception {
		when(rolGameService.findAllBySagaId("saga-id")).thenReturn(List.of());
