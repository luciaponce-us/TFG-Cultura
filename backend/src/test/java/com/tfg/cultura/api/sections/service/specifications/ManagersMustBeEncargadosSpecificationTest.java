...
public class ManagersMustBeEncargadosSpecificationTest {
    private ManagersMustBeEncargadosSpecification specification;

    @BeforeEach
    void setUp() {
        specification = new ManagersMustBeEncargadosSpecification();
    }

    @Test
    void should_not_throw_when_all_users_are_encargados() {
        Set<User> managers = Set.of(
                createUser("manager1", Role.ENCARGADO),
                createUser("manager2", Role.ENCARGADO));
    ...
    void should_throw_when_a_user_is_not_encargado() {
        ...
        InvalidManagerRoleException exception = assertThrows(
    ...
    void should_throw_when_multiple_users_are_not_encargados() {
        Set<User> managers = Set.of(
                createUser("manager1", Role.ENCARGADO),
                createUser("collaborator", Role.COLABORADOR),
                createUser("coordinator", Role.COORDINADOR));
