...
class MovieControllerTest extends BaseControllerTest {
	...
	private MockMultipartFile mockMoviePart(MovieRequest request) throws Exception {
		...
		return new MockMultipartFile("item", "movie.json", MediaType.APPLICATION_JSON_VALUE,
	...
	void should_create_movie_successfully_without_image() throws Exception {
		...
		mockMvc.perform(multipart(BASE_URL).file(moviePart)).andExpect(status().isCreated())
	...
	void should_create_movie_successfully_with_image() throws Exception {
		...
		mockMvc.perform(multipart(BASE_URL).file(moviePart).file(imagePart)).andExpect(status().isCreated())
				.andExpect(jsonPath("$.id").value(movieResponse.getId()))
				.andExpect(jsonPath("$.name").value(movieResponse.getName()))
				.andExpect(jsonPath("$.format").value(movieResponse.getFormat()));
	...
	void should_return_conflict_when_movie_already_exists() throws Exception {
		...
		mockMvc.perform(multipart(BASE_URL).file(moviePart)).andExpect(status().isConflict())
	...
	void should_return_bad_request_when_create_request_is_invalid() throws Exception {
		...
		mockMvc.perform(multipart(BASE_URL).file(moviePart)).andExpect(status().isBadRequest());
	...
	void should_get_movie_by_id() throws Exception {
		...
		mockMvc.perform(get(MOVIE_URL, movie.getId())).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(movie.getId())).andExpect(jsonPath("$.name").value(movie.getName()))
	...
	void should_get_paginated_movies() throws Exception {
		Page<MovieResponse> page = new PageImpl<>(List.of(movieResponse), PageRequest.of(0, 10), 1);

		when(movieService.getAll(PageRequest.of(0, 10), null, null)).thenReturn(page);

		mockMvc.perform(get(BASE_URL).param("page", "0").param("size", "10")).andExpect(status().isOk())
				.andExpect(jsonPath("$.content").isArray()).andExpect(jsonPath("$.content.length()").value(1))
				.andExpect(jsonPath("$.totalElements").value(1)).andExpect(jsonPath("$.size").value(10))
	...
	void should_delete_movie_successfully() throws Exception {
		mockMvc.perform(delete(MOVIE_URL, movie.getId())).andExpect(status().isNoContent());
	...
	void should_update_movie_successfully() throws Exception {
		...
		mockMvc.perform(multipart(MOVIE_URL, movie.getId()).file(moviePart).with(request -> {
			request.setMethod("PUT");
			return request;
		})).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(movie.getId()))
	...
	void should_return_conflict_when_updating_movie_that_already_exists() throws Exception {
		...
		mockMvc.perform(multipart(MOVIE_URL, movie.getId()).file(moviePart).with(request -> {
			request.setMethod("PUT");
			return request;
		})).andExpect(status().isConflict()).andExpect(jsonPath("$.errors.movie").exists());
