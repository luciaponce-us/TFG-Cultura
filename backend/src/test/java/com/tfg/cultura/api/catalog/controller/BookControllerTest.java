...
class BookControllerTest extends BaseControllerTest {
	...
	private MockMultipartFile mockBookPart(BookRequest request) throws Exception {
		return new MockMultipartFile("item", "book.json", MediaType.APPLICATION_JSON_VALUE,
	...
	void should_get_book_by_id() throws Exception {
		...
		mockMvc.perform(get(BOOK_URL, book.getId())).andExpect(status().isOk())
				.andExpect(jsonPath("$.id").value(book.getId())).andExpect(jsonPath("$.name").value(book.getName()))
	...
	void should_get_books_by_type() throws Exception {
		...
		mockMvc.perform(get(BOOKS_BY_TYPE_URL, "MANGA").param("page", "0").param("size", "10"))
	...
	void should_delete_book_successfully() throws Exception {
		mockMvc.perform(delete(BOOK_URL, book.getId())).andExpect(status().isNoContent());
	...
	void should_update_book_successfully() throws Exception {
		...
		mockMvc.perform(multipart(BOOK_URL, book.getId()).file(bookPart).with(request -> {
			...
			return request;
		})).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(book.getId()))
