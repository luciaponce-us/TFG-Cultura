...
public class BoardGameCsvParser extends ItemCsvParser {
    ...
    public List<BoardGame> loadBoardGamesExpansionFromCsv(Map<String, Section> sectionsByName,
        ...
        return loadCsv(CSV_FILE_PATH_EXPANSION, line -> mapLine(line, sectionsByName, categoriesByName, baseGamesByName));
    }

    private BoardGame mapLine(String line, Map<String, Section> sectionsByName,
            Map<String, Category> categoriesByName, Map<String, BoardGame> baseGamesByName) {
        ...
        Set<BoardGameType> types = parseList(parts[18]).stream()
                .map(BoardGameType::valueOf)
                .collect(Collectors.toSet());

        return builder
                .minPlayers(parseInteger(parts[14]))
                .maxPlayers(parseInteger(parts[15]))
                .playTime(parseInteger(parts[16]))
                .complexity(Complexity.valueOf(parts[17]))
                .types(types)
                .baseGame(getBaseGame(parts[16], baseGamesByName))
                .build();
    }
