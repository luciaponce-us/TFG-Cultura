...
public class SeriesCsvParser extends ItemCsvParser {
	...
	public List<Series> loadSeriesFromCsv(Map<String, Section> sectionsByName, Map<String, Category> categoriesByName) {
	...
	private Series mapLine(String line, Map<String, Section> sectionsByName, Map<String, Category> categoriesByName) {
		...
		SeriesInfo seriesInfo = SeriesInfo.builder().releaseDate(parseLocalDate(parts[16]))
				.numberOfSeasons(parseInteger(parts[17])).status(SeriesStatus.valueOf(clean(parts[18]))).build();

		List<Season> seasons = parseSeasons(parts[19]);

		return builder.format(Format.valueOf(clean(parts[14]))).numberOfDiscs(parseInteger(parts[15]))
				.seriesInfo(seriesInfo).seasons(seasons).build();
	}
	...
	private List<Season> parseSeasons(String value) {
		...
		String[] seasonStrings = value.replace("[", "").replace("]", "").replace("(", "").split("\\)");
	...
	private Season parseSeason(String value) {
		...
		return Season.builder().seasonNumber(seasonNumber).seasonPart(seasonPart).trailerUrl(trailerUrl).build();
	}
