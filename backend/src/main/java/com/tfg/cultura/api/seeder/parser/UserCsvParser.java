...
public class UserCsvParser extends CsvParser {
    ...
    private User mapLine(String line) {
        ...
        return User.builder()
                .username(clean(parts[0]))
                .password(passwordEncoder.encode(clean(parts[1])))
                .name(clean(parts[2]))
                .surname(clean(parts[3]))
                .dni(clean(parts[4]))
                .phone(clean(parts[5]))
                .email(clean(parts[6]))
                .role(Role.valueOf(clean(parts[7])))
                .paymentReceipt(clean(parts[8]))
                .active(Boolean.parseBoolean(clean(parts[9])))
                .build();
    }
}
