...
public class UserAuthService {
	...
	public UserResponse register(UserRegisterRequest request, MultipartFile avatar, MultipartFile paymentReceipt)
		...
		logger.info("Se va a intentar subir el PDF de la carta de pago para el usuario {}",
				LoggerSanitizer.sanitize(request.getUsername()));
		...
		User user = User.builder().username(request.getUsername())
				.password(passwordEncoder.encode(request.getPassword())).name(request.getName())
				.surname(request.getSurname()).dni(request.getDni()).phone(request.getPhone()).email(request.getEmail())
				.avatar(avatarUrl).paymentReceipt(paymentReceiptUrl).build();
	...
	public String login(UserLoginRequest request)
		...
		return jwtService.generateToken(userDetails.getUsername(), userDetails.getRole(), userDetails.getId());
	}
