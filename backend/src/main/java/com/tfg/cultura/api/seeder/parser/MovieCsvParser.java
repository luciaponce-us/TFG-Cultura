...
public class MovieCsvParser extends ItemCsvParser {
	...
	public List<Movie> loadMoviesFromCsv(Map<String, Section> sectionsByName, Map<String, Category> categoriesByName,
	...
	private Movie mapLine(String line, Map<String, Section> sectionsByName, Map<String, Category> categoriesByName,
		...
		MovieInfo movieInfo = MovieInfo.builder()
				.releaseDate(parseNullableString(parts[16]) != null ? parseLocalDate(parts[16]) : null)
				.trailerUrl(parseNullableString(parts[17])).saga(getSaga(parts[18], sagasByName)).build();

		return builder.format(Format.valueOf(clean(parts[14]))).numberOfDiscs(Integer.parseInt(parts[15]))
				.movieInfo(movieInfo).build();
	}
