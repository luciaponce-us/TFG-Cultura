...
public abstract class CsvParser {
	...
	protected <T> List<T> loadCsv(String path, Function<String, T> mapper) {
		...
		try (InputStream is = resource.getInputStream();
			...
			return reader.lines().skip(1).map(mapper).toList();
	...
	protected Set<User> getUsers(List<String> usernames, Map<String, User> usersByUsername) {

		return usernames.stream().map(username -> {
			User user = usersByUsername.get(username);

			if (user == null) {
				throw new IllegalStateException("No existe el usuario '" + username + "'");
			}
