...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) {
		String[] adminRoles = appProperties.adminRoles().stream().map(Role::name).toArray(String[]::new);

		String[] superAdminRoles = List.of(Role.COORDINADOR.name(), Role.SECRETARIO.name()).toArray(new String[0]);

		http.cors(Customizer.withDefaults()).csrf(csrf -> csrf.disable()) // NOSONAR: CSRF deshabilitado porque la API
																			// es stateless y usa JWT en
																			// headers (no cookies)
				.sessionManagement(sm -> sm.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
				.authorizeHttpRequests(auth -> auth
						// Rutas públicas
						.requestMatchers("/", "/api", "/api/").permitAll()
						.requestMatchers(HttpMethod.GET, "/api/dummy", "/api/dummy/**").permitAll().requestMatchers(
								// Swagger and API docs
								"/v3/api-docs/**", "/swagger-ui/**", "/swagger-ui.html", "/api/docs", "/docs/**",
								"/docs", "/api/docs/**", "/api/swagger-ui/**")
						.permitAll()
						...
						.requestMatchers(HttpMethod.DELETE, "/api/catalog/**").hasAnyRole(adminRoles).anyRequest()
						.authenticated())
