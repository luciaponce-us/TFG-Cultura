...
public class UserFileService {
    ...
    protected String uploadPaymentReceiptPdf(String userId, MultipartFile file) throws FileUploadException {
        ...
        return fileService.uploadPdf(
                "payment",
                userId,
                file,
                PAYMENT_FOLDER,
                PAYMENT_RECEIPT_PLACEHOLDER,
                logger,
                "paymentReceipt");
    }
    ...
    protected void validatePaymentReceipt(MultipartFile pdf) {
        if (pdf == null || pdf.isEmpty())
            throw new ValidationException(logger,Map.of("paymentReceipt", "El archivo de carta de pago es obligatorio"));
