...
public abstract class ItemCsvParser extends CsvParser {
	...
	protected void fillItemFields(Item.ItemBuilder<?, ?> builder, String[] parts,
			Map<String, Category> categoriesByName, Map<String, Section> sectionsByName) {
		...
		builder.name(clean(parts[0])).description(clean(parts[1])).imageUrl(parseNullableString(parts[2]))
				...
				.copies(parseInteger(parts[9])).availableCopies(parseInteger(parts[10]))
				.loanDays(parseInteger(parts[11])).section(section).categories(categories);
	}

	protected Section getSection(String name, Map<String, Section> sectionsByName) {
	...
	protected Set<Category> getCategories(List<String> names, Map<String, Category> categoriesByName) {

		return names.stream().map(name -> {
			Category category = categoriesByName.get(name);

			if (category == null) {
				throw new IllegalStateException("No existe la categoría '" + name + "'");
			}
	...
	protected Saga getSaga(String name, Map<String, Saga> sagasByName) {
