...
import type { Platform, VideoGameErrors, VideoGameRequest } from "../types/videogame";
...
function validateReleaseDate(value: string): string | undefined {
    ...
    if (!isPastOrPresentDate(value)) return "La fecha de lanzamiento no puede ser futura.";
    return;
}

function validateTrailerUrl(value: string): string | undefined {
    if (value.length > MAX_LENGTH.TRAILER_URL) return `La URL del tráiler no puede superar los ${MAX_LENGTH.TRAILER_URL} caracteres.`;
    return;
}